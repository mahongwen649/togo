...
func TestOpenAIProviderGenerateText(t *testing.T) {
	...
	text, err := provider.GenerateText(channels.Snapshot{
		ChannelName: "OpenAI",
		BaseURL:     server.URL,
		APIKey:      "sk-test",
		ModelID:     "gpt-4o",
	...
	if !strings.Contains(string(bodyBytes), "data:image/png;base64,ZmFrZS1pbWFnZQ") {
		t.Fatalf("request body missing image data URL: %s", string(bodyBytes))
	}
}

func TestOpenAIProviderGenerateTextReturnsUpstreamError(t *testing.T) {
	...
	provider := OpenAIProvider{Client: server.Client()}
	_, err := provider.GenerateText(channels.Snapshot{
		BaseURL: server.URL,
		APIKey:  "sk-test",
		ModelID: "gpt-4o",
	}, TextRequest{Prompt: "describe", Image: []byte("fake-image"), MimeType: "image/png"})
	if err == nil {

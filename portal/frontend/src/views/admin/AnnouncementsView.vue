    ...
    <TablePageLayout>
      <template #filters>
        <div class="h5-filter-grid flex flex-wrap items-center gap-3">
          ...
          <div class="h5-filter-actions flex flex-1 flex-wrap items-center justify-end gap-2">
            <button
      ...
      <template #table>
        ...
        >
          ...
          <template #empty>
            <EmptyState
              :title="loadFailed ? t('empty.noData') : t('admin.announcements.empty')"
              :description="loadFailed ? t('admin.announcements.failedToLoad') : t('admin.announcements.emptyDescription')"
...
async function loadAnnouncements() {
  ...
  try {
    ...
    const res = await announcementsAPI.list(pagination.page, pagination.page_size, {
...
async function handleSave() {
  ...
  try {
    ...
    await announcementsAPI.update(original.id, payload)
    appStore.showSuccess(t('common.success'))
    showEditDialog.value = false
    editingAnnouncement.value = null
    await loadAnnouncements()
...
async function confirmDelete() {
  ...
  try {
    await announcementsAPI.delete(deletingAnnouncement.value.id)
    appStore.showSuccess(t('common.success'))

    ...
    <TablePageLayout>
      ...
      <template #filters>
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="h5-filter-grid flex flex-1 flex-wrap items-center gap-3">
            <!-- Search Box -->
            ...
            <div v-if="authStore.isFullAdmin && visibleFilters.has('apiKeyGroup')" class="w-full sm:w-44">
              <Select
          ...
          <div class="h5-filter-actions flex flex-wrap items-center justify-end gap-2">
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <div class="relative" ref="filterDropdownRef">
                ...
                >
                  <!-- Built-in filters -->
                  <button
                    v-for="filter in visibleBuiltInFilters"
                    :key="filter.key"
      ...
      <template #table>
        ...
        >
          <template #cell-email="{ value, row }">
            <div :class="['flex min-w-0 items-center', isMobileH5 ? 'gap-1' : 'gap-2']">
              <div
                v-if="!isMobileH5"
                class="flex h-8 w-8 items-center justify-center rounded-full bg-primary-100 dark:bg-primary-900/30"
              >
                <span class="text-sm font-medium text-primary-700 dark:text-primary-300">
                  {{ getUserAvatarInitial(row) }}
                </span>
              </div>
              <span
                v-if="isMobileH5"
                class="block max-w-[8rem] truncate text-xs font-medium text-gray-900 dark:text-white"
                :title="row.username || value"
                data-test="mobile-username"
              >
                {{ row.username || value || '-' }}
              </span>
          ...
          <template #cell-role="{ value, row }">
            <div class="flex flex-wrap items-center gap-1.5">
              ...
              <span
                v-if="row.is_group_manager"
                class="badge badge-primary"
                :title="t('admin.users.groupManagerRole')"
              >
                {{ t('admin.users.groupManagerRole') }}
              </span>
            </div>
          </template>
              ...
              <div class="group relative">
                <button
                  :class="[
                    ...
                    isMobileH5 ? 'text-xs no-underline' : ''
                  ]"
          ...
          <template #cell-status="{ value }">
            <div class="flex items-center gap-1.5">
              ...
              ></span>
              <span :class="[isMobileH5 ? 'text-xs' : 'text-sm', 'text-gray-700 dark:text-gray-300']">
          ...
          <template #cell-last_used_at="{ value }">
            <span :class="[isMobileH5 ? 'text-xs' : 'text-sm', 'whitespace-nowrap text-gray-500 dark:text-dark-400']">
...
import { getUserAvatarInitial } from '@/utils/userDisplay'
...
const MOBILE_USER_COLUMNS = new Set(['email', 'id', 'role', 'balance', 'status', 'last_used_at'])
...
const columns = computed<Column[]>(() =>
  allColumns.value.filter(col =>
    (col.key === 'email' || col.key === 'actions' || !hiddenColumns.has(col.key))
      && (!isMobileH5.value || MOBILE_USER_COLUMNS.has(col.key))
  )
)
...
const loadAllGroupsForApiKeyFilter = async () => {
  ...
  try {
    allGroupsForApiKeyFilter.value = await groupsAPI.getAllIncludingInactive()
  } catch (e) {
...
const visibleBuiltInFilters = computed(() =>
  authStore.isFullAdmin
    ? builtInFilters.value
    : builtInFilters.value.filter((filter) => filter.key !== 'apiKeyGroup')
)
...
const loadUsersSecondaryData = async (
  ...
  if (hasVisibleUsageColumn.value) {
    tasks.push(
      (async () => {
        try {
          const usageResponse = await dashboardAPI.getBatchUsersUsage(userIds)
  ...
  if (attributeDefinitions.value.length > 0 && hasVisibleAttributeColumns.value) {
    tasks.push(
      (async () => {
        try {
          const attrResponse = await userAttributesAPI.getBatchUserAttributes(userIds)
          if (signal?.aborted) return
          if (typeof expectedSeq === 'number' && expectedSeq !== secondaryDataSeq) return
        ...
        try {
          ...
          for (let i = 0; i < userIds.length; i += CHUNK) {
            ...
            const results = await Promise.allSettled(
              chunk.map((id) => usersAPI.getPlatformQuotas(id))
            )
            if (signal?.aborted) return
            if (typeof expectedSeq === 'number' && expectedSeq !== secondaryDataSeq) return
...
const toggleBuiltInFilter = (key: string) => {
  if (key === 'apiKeyGroup' && !authStore.isFullAdmin) return
  if (visibleFilters.has(key)) {
    ...
    if (key === 'apiKeyGroup' && authStore.isFullAdmin) loadAllGroupsForApiKeyFilter()
  }
...
onMounted(async () => {
  ...
  if (authStore.isFullAdmin && visibleFilters.has('apiKeyGroup')) {

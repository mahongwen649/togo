  ...
  <AppLayout>
    <div :class="isMobileH5 ? 'space-y-4' : 'space-y-6'">
      ...
      <div class="space-y-4">
        ...
        />
        <div class="grid grid-cols-1 lg:grid-cols-2" :class="isMobileH5 ? 'gap-4' : 'gap-6'">
      ...
      <div class="card">
        ...
        <UsageFilters v-model="filters" ref="usageFiltersRef" flat :mode="activeTab" class="border-b border-gray-100 dark:border-dark-700/50" :start-date="startDate" :end-date="endDate" :exporting="exporting" :model-options="modelNameOptions" :show-cleanup="authStore.isFullAdmin" @change="applyFilters" @refresh="refreshData" @reset="resetFilters" @cleanup="openCleanupDialog" @export="exportToExcel">
        ...
        <div v-if="activeTab === 'errors'" class="overflow-hidden rounded-b-2xl">
          <AdminErrorRequestsTable
            :rows="errorRows"
            :total="errorTotal"
...
import AppLayout from '@/components/layout/AppLayout.vue'; import Pagination from '@/components/common/Pagination.vue'; import ChartRangeToolbar from '@/components/common/ChartRangeToolbar.vue'
import { useDeviceMode } from '@/composables/useDeviceMode'
import UsageStatsCards from '@/components/admin/usage/UsageStatsCards.vue'; import UsageFilters from '@/components/admin/usage/UsageFilters.vue'
import UsageTable from '@/components/admin/usage/UsageTable.vue'; import UsageExportProgress from '@/components/admin/usage/UsageExportProgress.vue'
import AdminErrorRequestsTable from '@/components/admin/usage/AdminErrorRequestsTable.vue'
...
import type { AdminUsageLog, TrendDataPoint, ModelStat, GroupStat, EndpointStat, AdminUser } from '@/types'; import type { AdminErrorRequest, AdminUsageStatsResponse, AdminUsageQueryParams } from '@/api/admin/usage'
...
const handleUserClick = async (userId: number) => {
  try {
    const user = await usersAPI.getById(userId, true)
...
const loadStats = async (force = false) => {
  ...
  try {
    const requestType = filters.value.request_type
    const legacyStream = requestType ? requestTypeToLegacyStream(requestType) : filters.value.stream
    const s = await adminUsageAPI.getStats({
      ...filters.value,
      stream: legacyStream === null ? undefined : legacyStream,
      ...(force ? { nocache: 1 } : {}),
    })
...
const loadModelStats = async (source: ModelDistributionSource, force = false) => {
  ...
  try {
    ...
    const response = await dashboardAPI.getModelStats({ ...baseParams, model_source: source })
...
const loadChartData = async () => {
  ...
  try {
    const requestType = filters.value.request_type
    const legacyStream = requestType ? requestTypeToLegacyStream(requestType) : filters.value.stream
    const snapshot = await dashboardAPI.getSnapshotV2({
      start_date: filters.value.start_date || startDate.value,
      end_date: filters.value.end_date || endDate.value,
...
const USER_AGENT_VISIBLE_MIGRATION_KEY = 'usage-user-agent-visible-v1'
...
const loadSavedColumns = () => {
  ...
  // Existing installations persisted user_agent as hidden. Reveal it once while
  // preserving every other saved column preference; later user changes still win.
  try {
    if (localStorage.getItem(USER_AGENT_VISIBLE_MIGRATION_KEY) !== '1') {
      hiddenColumns.delete('user_agent')
      localStorage.setItem(HIDDEN_COLUMNS_KEY, JSON.stringify([...hiddenColumns]))
      localStorage.setItem(USER_AGENT_VISIBLE_MIGRATION_KEY, '1')
    }
  } catch (e) {
    console.error('Failed to migrate user-agent column visibility:', e)
  }
}
...
const switchTab = (tab: DetailTab) => {
  activeTab.value = tab
  if (tab === 'errors' && errorRows.value.length === 0) loadAdminErrors()
  if (tab === 'ranking') rankingMounted.value = true
}

const errorRows = ref<AdminErrorRequest[]>([])
...
const loadAdminErrors = async () => {
  ...
  try {
    const response = await adminUsageAPI.listErrors({
...
const handleErrorSort = (key: string, order: 'asc' | 'desc') => {
  errorSortBy.value = key
  errorSortOrder.value = order
  errorPage.value = 1
  loadAdminErrors()
}
const handleErrorPageChange = (page: number) => { errorPage.value = page; loadAdminErrors() }
...
onUnmounted(() => { abortController?.abort(); errorAbortController?.abort(); exportAbortController?.abort(); document.removeEventListener('click', handleColumnClickOutside) })

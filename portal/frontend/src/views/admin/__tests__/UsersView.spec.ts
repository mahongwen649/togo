...
vi.mock('@/composables/useDeviceMode', () => ({
  useDeviceMode: () => ({ isMobileH5: mobileMode })
}))
...
vi.mock('@/api/admin/userAttributes', () => ({
  userAttributesAPI: {
    listEnabledDefinitions,
    getBatchUserAttributes
  },
...
describe('admin UsersView', () => {
  ...
  it('uses device mode to render a dense username-first mobile table without actions', async () => {
    ...
    expect(wrapper.get('[data-test="columns"]').text()).toBe('email,id,role,balance,status,last_used_at')
    expect(wrapper.get('[data-test="mobile-username"]').text()).toBe('scoped-user')
    expect(wrapper.find('.h-8.w-8.rounded-full').exists()).toBe(false)
  })

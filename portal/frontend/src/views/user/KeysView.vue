    ...
    <TablePageLayout>
      <template #filters>
        <div class="flex flex-col gap-3">
          <div class="h5-filter-grid flex flex-wrap items-center gap-3">
            <SearchInput
          ...
          <EndpointPopover
            :api-base-url="primaryApiBaseUrl"
      ...
      <template #actions>
        ...
        <div class="relative" ref="columnDropdownRef">
          ...
          <div v-if="showColumnDropdown" class="absolute right-0 top-full z-50 mt-1 max-h-80 w-48 overflow-y-auto rounded-lg border border-gray-200 bg-white py-1 shadow-lg dark:border-dark-600 dark:bg-dark-800">
            ...
            >
              <span>{{ col.label }}</span>
              <span>{{ isColumnVisible(col.key) ? '✓' : '' }}</span>
            </button>
          </div>
        </div>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-last_used_ip="{ value }">
            <span v-if="value" data-test="last-used-ip" class="text-sm text-gray-500 dark:text-dark-400">{{ value }}</span>
            <span v-else class="text-sm text-gray-400 dark:text-dark-500">-</span>
          </template>
...
const toggleableColumns = computed(() => allColumns.value.filter((col) => !ALWAYS_VISIBLE_COLUMNS.has(col.key)))
const columns = computed<Column[]>(() => allColumns.value.filter((col) => ALWAYS_VISIBLE_COLUMNS.has(col.key) || !hiddenColumns.has(col.key)))
...
const loadSavedColumns = () => {
  ...
  try {
    ...
    const valid = new Set(allColumns.value.map((col) => col.key))
    const values = saved ? JSON.parse(saved) as string[] : DEFAULT_HIDDEN_COLUMNS
    values.filter((key) => valid.has(key) && !ALWAYS_VISIBLE_COLUMNS.has(key)).forEach((key) => hiddenColumns.add(key))
    const storedVersion = Number(localStorage.getItem(COLUMN_SETTINGS_VERSION_KEY) ?? '1')
    if (saved && storedVersion < COLUMN_SETTINGS_VERSION) {
      for (let version = storedVersion + 1; version <= COLUMN_SETTINGS_VERSION; version++) {
        for (const key of VERSION_NEW_HIDDEN_COLUMNS[version] ?? []) {
          if (valid.has(key) && !ALWAYS_VISIBLE_COLUMNS.has(key)) {
...
const primaryApiBaseUrl = computed(() => publicSettings.value?.api_base_url || getPublicGatewayBaseURL())
...
function selectDefaultGroupForCreate() {
  if (showEditModal.value || !showCreateModal.value || formData.value.group_id !== null) return
  formData.value.group_id = groupOptions.value[0]?.value ?? null
}
...
const executeCcsImport = (row: ApiKey, clientType: CcSwitchClientType) => {
	const baseUrl = publicSettings.value?.api_base_url || getPublicGatewayBaseURL()
  const platform = row.group?.platform || 'anthropic'

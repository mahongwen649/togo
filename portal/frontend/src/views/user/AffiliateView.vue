            ...
            <div class="space-y-2">
              ...
              <div class="flex flex-col items-stretch gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 dark:border-dark-700 dark:bg-dark-900 sm:flex-row sm:items-center">
                ...
                <button class="btn btn-secondary btn-sm w-full sm:w-auto sm:shrink-0" @click="copyCode">
                  <Icon name="copy" size="sm" /><span>{{ t('affiliate.copyCode') }}</span>
                </button>
              </div>
            </div>
            <div class="space-y-2">
              ...
              <div class="flex flex-col items-stretch gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 dark:border-dark-700 dark:bg-dark-900 sm:flex-row sm:items-center">
                ...
                <button class="btn btn-secondary btn-sm w-full sm:w-auto sm:shrink-0" @click="copyInviteLink">
                  <Icon name="copy" size="sm" /><span>{{ t('affiliate.copyLink') }}</span>
                </button>
              </div>
            </div>
          </div>
        ...
        <div class="card p-6">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button class="btn btn-primary" :disabled="transferring || detail.aff_quota <= 0" @click="transferQuota">
              <Icon :name="transferring ? 'refresh' : 'dollar'" size="sm" :class="transferring ? 'animate-spin' : ''" />
        ...
        <div class="card p-6">
          ...
          <div v-else class="mt-4 overflow-x-auto">
            <table class="w-full min-w-[560px] text-left text-sm">
              ...
              <tbody><tr v-for="item in detail.invitees" :key="item.user_id" class="border-b border-gray-100 last:border-b-0 dark:border-dark-800">
...
const inviteLink = computed(() => detail.value
  ? `${typeof window === 'undefined' ? '' : window.location.origin}/register?aff=${encodeURIComponent(detail.value.aff_code)}`
  : '')
const formattedRebateRate = computed(() => {
  const rounded = Math.round((detail.value?.effective_rebate_rate_percent ?? 0) * 100) / 100
  return Number.isInteger(rounded) ? String(rounded) : rounded.toString()
})

function formatCount(value: number): string { return value.toLocaleString() }
async function loadAffiliateDetail(silent = false): Promise<void> {
  if (!silent) loading.value = true
  try { detail.value = await userAPI.getAffiliateDetail() }
  catch (error) { appStore.showError(extractApiErrorMessage(error, t('affiliate.loadFailed'))) }

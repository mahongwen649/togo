      ...
      <template #filters>
        <div class="flex flex-col gap-3">
          <div class="flex flex-col gap-3 2xl:flex-row 2xl:items-center 2xl:justify-between">
            <div class="h5-filter-grid grid w-full grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-[260px_160px_144px_152px] 2xl:w-auto">
              <div class="min-w-0">
                <SearchInput
            ...
            <div class="h5-filter-actions flex flex-wrap items-center justify-start gap-2 sm:justify-end 2xl:flex-shrink-0">
      ...
      <form class="space-y-5" @submit.prevent="submitJob">
        ...
        <div class="space-y-3">
          ...
          <div v-else class="rounded-lg border border-dashed border-gray-200 px-3 py-6 text-center text-sm text-gray-500 dark:border-dark-700 dark:text-gray-400">
	        ...
	        <div v-if="submitting" class="rounded-lg border border-teal-200 bg-teal-50 p-3 text-sm leading-6 text-teal-800 dark:border-teal-800 dark:bg-teal-950/30 dark:text-teal-100">
...
const endpointBase = computed(() => {
  ...
  if (typeof window !== 'undefined') return getPublicGatewayBaseURL()
...
function batchImageText(key: BatchImageTextKey) {
  const zh: Record<BatchImageTextKey, string> = {
    ...
    noCompatibleAccount: '当前密钥所属分组没有可用的批量生图调度资源。请联系管理员检查：该分组是否已同步可用的 Gemini API Key 或 Vertex 服务配置，以及资源是否支持所选模型。',
    unsupportedProvider: '这个任务使用的批量生图通道当前不可用。请联系管理员检查批量生图通道配置。',
    providerSubmitFailed: '批量生图任务提交到服务端资源失败。请联系管理员检查资源状态、模型权限或服务状态。',
  ...
  const en: Record<BatchImageTextKey, string> = {
    ...
    noCompatibleAccount: 'No usable batch image scheduling resource is available for this key’s group. Contact an administrator to check synced Gemini API key or Vertex service configuration and model support.',
    ...
    providerSubmitFailed: 'The batch image job failed to submit to the service resource. Contact an administrator to check resource status, model permission, or provider status.',

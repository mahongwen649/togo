...
This directory contains the Vue Router configuration for the trimmed Portal frontend. The router keeps only the Core/Portal split target pages and redirects removed legacy domains to the appropriate dashboard.
...
| ----------- | ------------ | ---------------------- |
| `/home`     | HomeView     | Login/register landing |
| `/login`    | -            | Redirects to `/home?mode=login` |
| `/register` | -            | Redirects to `/home?mode=register` |
...
| `/docs`      | DocsView      | API usage guide              |
| `/models`    | ModelMarketView | Available models and pricing |
...
| `/admin/announcements` | AnnouncementsView | Announcement management     |
| `/admin/usage`     | AdminUsageView     | Usage analytics                 |
| `/admin/companies` | CompaniesView      | Portal company management       |
| `/admin/finance`   | FinanceView        | Portal finance aggregation      |

### Removed Legacy Routes

Legacy subscription, payment, channel, account, proxy, group-management, system-settings, and ops paths are intentionally not feature routes. They are kept only in the removed-route prefix list so old bookmarks land safely on `/dashboard` or `/admin/dashboard`.
...
2. **Checks Authentication**:
   ...
   - Redirects to `/home?mode=login` if not authenticated
...
1. **Public Route Access**: Visit `/home?mode=login` without authentication
...
5. **Removed Route Redirect**: Visit `/admin/settings` or `/purchase` (should redirect to dashboard)
6. **404 Handling**: Visit non-existent route (should show 404 page)

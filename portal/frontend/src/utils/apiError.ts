...
export function extractApiErrorCode(err: unknown): string | undefined {
  ...
  const code = e.reason ?? e.code ?? e.response?.data?.reason ?? e.response?.data?.code
 ...
 * Example: "certSerial" → t('fields.certSerial') when available; otherwise "certSerial".
 */
 ...
 * localized UI labels when a generic field-label i18n key exists.
...
function localizeMetadata(metadata: Record<string, unknown>, t: TranslateFn): Record<string, unknown> {
  ...
  if (typeof out.key === 'string') {
    out.key = tryTranslate(t, `fields.${out.key}`, out.key)
  }
  if (typeof out.keys === 'string') {
    out.keys = out.keys
      .split('/')
      .map(k => tryTranslate(t, `fields.${k}`, k))
      .join(' / ')
  }
  return out
}
...
export function extractI18nErrorMessage(
  ...
  let code = extractApiErrorCode(err)
  if (namespace === 'auth.errors') {
    const rawMessage = extractApiErrorMessage(err, '').toLowerCase()
    if (rawMessage.includes('email verification is required')) {
      code = 'EMAIL_VERIFY_REQUIRED'
    } else if (rawMessage.includes('verification code is required')) {
      code = 'VERIFY_CODE_REQUIRED'
    } else if (
      rawMessage.includes('invalid verification code') ||
      rawMessage.includes('verification code is invalid') ||

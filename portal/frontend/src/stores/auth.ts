...
import { getManagementScope, type ManagementScope } from '@/api/admin/scope'
...
export const useAuthStore = defineStore('auth', () => {
  ...
  const managementScope = ref<ManagementScope | null>(null)
  ...
  const isFullAdmin = computed(() => managementScope.value?.is_admin === true || isAdmin.value)
  const isGroupManager = computed(() => managementScope.value?.is_group_manager === true)
  const isCompanyManager = computed(() => managementScope.value?.is_company_manager === true)
  const canAccessAdminArea = computed(() => isFullAdmin.value || isGroupManager.value || isCompanyManager.value)
  const managedGroupIds = computed(() => managementScope.value?.managed_group_ids ?? [])
  const managedCompanyIds = computed(() => managementScope.value?.managed_company_ids ?? [])
   ...
   */
  async function setAuthFromResponse(response: AuthResponse): Promise<void> {
    ...
    await refreshManagementScope().catch(() => {})
  ...
  async function refreshManagementScope(): Promise<ManagementScope | null> {
    ...
    try {
      managementScope.value = await getManagementScope()
    } catch {
      managementScope.value = null
    }
    return managementScope.value
  }
  ...
  return {
    ...
    managementScope: readonly(managementScope),

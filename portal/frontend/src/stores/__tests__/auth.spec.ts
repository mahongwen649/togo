...
const mockGetManagementScope = vi.fn()
...
vi.mock('@/api/admin/scope', () => ({
  getManagementScope: (...args: any[]) => mockGetManagementScope(...args),
}))
...
describe('useAuthStore', () => {
  beforeEach(() => {
    ...
    vi.useFakeTimers()
    vi.clearAllMocks()
    mockGetManagementScope.mockRejectedValue({ status: 403 })
  })
  ...
  describe('login', () => {
    it('成功登录后设置 token 和 user', async () => {
      ...
      await store.login({ identifier: 'test@example.com', password: '123456' })
    ...
    it('登录失败时清除状态并抛出错误', async () => {
      ...
      await expect(store.login({ identifier: 'test@example.com', password: 'wrong' })).rejects.toThrow(
        'Invalid credentials'
      )
  ...
  describe('pending auth session', () => {
    ...
    it('preserves pending auth session when registration fails', async () => {
      ...
      await expect(
        store.register({ username: 'test-user', email: 'user@example.com', password: 'secret-123' })
  ...
  describe('isAdmin', () => {
    it('管理员用户返回 true', async () => {
      ...
      await store.login({ identifier: 'admin@example.com', password: '123456' })
  ...
  describe('management scope', () => {
    it('识别分组负责人管理范围', async () => {
      mockLogin.mockResolvedValue(fakeAuthResponse)
      mockGetManagementScope.mockResolvedValue({
        user_id: fakeUser.id,
        is_admin: false,
      ...
      expect(store.isAdmin).toBe(false)
      expect(store.isFullAdmin).toBe(false)
      expect(store.isGroupManager).toBe(true)
      expect(store.canAccessAdminArea).toBe(true)
      expect(store.managedGroupIds).toEqual([10, 20])
    })

    it('管理员保持完整管理权限', async () => {
      const adminResponse = { ...fakeAuthResponse, user: { ...fakeAdminUser } }
      mockLogin.mockResolvedValue(adminResponse)
      mockGetManagementScope.mockResolvedValue({
        user_id: fakeAdminUser.id,
        is_admin: true,
      ...
      expect(store.isAdmin).toBe(true)
      expect(store.isFullAdmin).toBe(true)
      expect(store.canAccessAdminArea).toBe(true)
    })

    it('登录返回前识别公司经理管理范围', async () => {
      mockLogin.mockResolvedValue(fakeAuthResponse)
      mockGetManagementScope.mockResolvedValue({
        user_id: fakeUser.id,
        is_admin: false,
      ...
      await store.login({ identifier: 'user3', password: '12345678' })

      expect(store.isCompanyManager).toBe(true)
      expect(store.canAccessAdminArea).toBe(true)
      expect(store.managedCompanyIds).toEqual([3])
    })
  })

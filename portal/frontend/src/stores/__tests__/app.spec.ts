...
describe('useAppStore', () => {
  ...
  describe('公开设置加载', () => {
    ...
    it('从 window.__APP_CONFIG__ 初始化', () => {
      ...
      expect(store.siteLogo).toBe('/logo.png?v=togo-20260725')
    ...
    it('fetchPublicSettings(force) 会同步更新运行时注入配置', async () => {
      vi.mocked(getPublicSettings).mockResolvedValue({
        ...
        site_logo: '',
        site_subtitle: '',
        api_base_url: ' https://api.togoapi.com/// ',
        contact_info: '',
        doc_url: '',
        home_content: '',
        hide_ccs_import_button: false,
      ...
      expect((window as any).__APP_CONFIG__.api_base_url).toBe('https://api.togoapi.com')
      expect(store.apiBaseUrl).toBe('https://api.togoapi.com')

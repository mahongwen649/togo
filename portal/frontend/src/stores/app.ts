...
import { normalizePublicGatewayBaseURL } from '@/api/url'

const DEFAULT_SITE_LOGO = '/logo.png?v=togo-20260725'
...
export const useAppStore = defineStore('app', () => {
  ...
  function applySettings(config: PublicSettings): PublicSettings {
    const normalizedConfig: PublicSettings = {
      ...config,
      site_logo: config.site_logo === '/logo.png' ? DEFAULT_SITE_LOGO : config.site_logo,
      api_base_url: normalizePublicGatewayBaseURL(config.api_base_url)
    }
    if (typeof window !== 'undefined') {
      window.__APP_CONFIG__ = normalizedConfig
    }
    cachedPublicSettings.value = normalizedConfig
    siteName.value = normalizedConfig.site_name || 'TogoAPI'

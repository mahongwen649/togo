  ...
  describe('请求拦截器', () => {
    ...
    it('无 token 时不附加 Authorization 头', async () => {
      ...
      expect(config.headers.get('Authorization')).toBeFalsy()
		...
		it('仅附加未过期的 Portal step-up token', async () => {
			const adapter = vi.fn().mockResolvedValue({
				status: 200,
				data: { code: 0, data: {} },
				headers: {},
			...
			apiClient.defaults.adapter = adapter
			sessionStorage.setItem('portal_step_up_token', 'step-token')
			sessionStorage.setItem('portal_step_up_expires_at', String(Date.now() + 60_000))

			await apiClient.get('/admin/proxies/data')
			expect(adapter.mock.calls[0][0].headers.get('X-Portal-Step-Up')).toBe('step-token')

			sessionStorage.setItem('portal_step_up_expires_at', String(Date.now() - 1))
			await apiClient.get('/admin/proxies/data')
			expect(adapter.mock.calls[1][0].headers.get('X-Portal-Step-Up')).toBeFalsy()
			expect(sessionStorage.getItem('portal_step_up_token')).toBeNull()
		})
    ...
    it('请求默认带 withCredentials 以支持跨域 cookie', async () => {
      ...
      await apiClient.post('/auth/login', { identifier: 'user@example.com', password: 'secret' })
    ...
    it('保留的用户 API 带 User UI 标记，已下线支付 API 不带', async () => {
      const adapter = vi.fn().mockResolvedValue({
        status: 200,
        data: { code: 0, data: {} },
        headers: {},
      ...
      await apiClient.get('/redeem/history')

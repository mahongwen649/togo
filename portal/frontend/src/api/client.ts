...
const attachStepUpToken = (config: InternalAxiosRequestConfig) => {
	try {
		const expiresAt = Number(sessionStorage.getItem('portal_step_up_expires_at') || 0)
		const token = sessionStorage.getItem('portal_step_up_token') || ''
		if (token && expiresAt > Date.now()) {
			config.headers['X-Portal-Step-Up'] = token
		} else {
			sessionStorage.removeItem('portal_step_up_token')
			sessionStorage.removeItem('portal_step_up_expires_at')
		}
	} catch {
		// Ignore unavailable session storage; the backend will request step-up again.
	}
}
  ...
  async (error: AxiosError<ApiResponse<unknown>>) => {
    ...
    if (error.response) {
      ...
      // Validate `data` shape to avoid HTML error pages breaking our error handling.
			let decodedData: unknown = data
			if (data instanceof Blob && data.type.includes('json')) {
				try {
					decodedData = JSON.parse(await readBlobText(data))
				} catch {
					decodedData = {}
				}
			}
			const apiData = (typeof decodedData === 'object' && decodedData !== null ? decodedData : {}) as Record<string, any>
...
function readBlobText(blob: Blob): Promise<string> {
	return new Promise((resolve, reject) => {
		...
		reader.onerror = () => reject(reader.error || new Error('failed to read response'))

...
let pendingGenerateOperation: { signature: string; key: string } | null = null
...
export async function generate(
  ...
  if (!pendingGenerateOperation || pendingGenerateOperation.signature !== signature) {
    pendingGenerateOperation = {
      signature,
      key: `redeem-generate-${crypto.randomUUID()}`
    }
  }
  try {
    const { data } = await apiClient.post<RedeemCode[]>('/admin/redeem-codes/generate', payload, {
      headers: { 'Idempotency-Key': pendingGenerateOperation.key }
    })
    pendingGenerateOperation = null
    return data
  } catch (error: any) {

...
export async function listMembers(
  ...
  const { data } = await apiClient.get<PaginatedResponse<GroupMember>>(
...
export async function listMemberCandidates(
  ...
  const { data } = await apiClient.get<PaginatedResponse<AdminUser>>(
    `/admin/groups/${id}/member-candidates`,
    { params: { search, page, page_size: pageSize } }
  )
  return data
}

export async function addMember(id: number, userId: number): Promise<{ message: string }> {
  const { data } = await apiClient.post<{ message: string }>(`/admin/groups/${id}/members`, {
    user_id: userId
  })
  return data
}
...
export async function addManager(id: number, userId: number): Promise<{ message: string }> {
  const { data } = await apiClient.post<{ message: string }>(
    `/admin/groups/${id}/managers/${userId}`
  )
  return data
}

export async function removeManager(id: number, userId: number): Promise<{ message: string }> {
  const { data } = await apiClient.delete<{ message: string }>(
    `/admin/groups/${id}/managers/${userId}`
  )
  return data

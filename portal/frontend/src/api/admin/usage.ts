...
export interface AdminErrorRequest {
  id: number
  created_at: string
  phase: string
  type: string
...
export interface AdminErrorRequestDetail extends AdminErrorRequest {
  error_body?: string
...
export async function listErrors(
  params: AdminErrorListParams,
  options?: { signal?: AbortSignal }
): Promise<PaginatedResponse<AdminErrorRequest>> {
  const { data } = await apiClient.get<PaginatedResponse<AdminErrorRequest>>(
...
export async function getErrorDetail(id: number): Promise<AdminErrorRequestDetail> {
  const { data } = await apiClient.get<AdminErrorRequestDetail>(

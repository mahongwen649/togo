...
const DEFAULT_PUBLIC_API_BASE_URL = 'https://api.togoapi.com'
...
export function normalizePublicGatewayBaseURL(value: unknown): string {
  return String(value || '').trim().replace(/\/+$/, '')
}

export function getPublicGatewayBaseURL(): string {
  const configured = String(import.meta.env.VITE_PUBLIC_API_BASE_URL || '').trim()
  return normalizePublicGatewayBaseURL(configured || DEFAULT_PUBLIC_API_BASE_URL)
}

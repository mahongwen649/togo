  ...
  <div class="card" :class="isMobileH5 ? 'p-3' : 'p-4'">
    ...
    >
      ...
      >
        ...
        <Icon name="chevronDown" size="sm" class="text-gray-400 transition-transform" :class="contentVisible ? 'rotate-180' : ''" />
      </button>
      <h3 v-else class="text-sm font-semibold text-gray-900 dark:text-white">
        {{ t('admin.dashboard.groupDistribution') }}
      </h3>
      <div
        v-if="showMetricToggle && contentVisible"
            ...
            <template v-for="group in displayGroupStats" :key="group.group_id">
              ...
              >
                ...
                <td v-if="showAccountCost && !isMobileH5" class="py-1.5 text-right text-orange-500 dark:text-orange-400">
                  ¥{{ formatCost(upstreamCostCny(group.account_cost)) }}
                </td>
...
import { upstreamCostCny } from '@/utils/portalCurrency'

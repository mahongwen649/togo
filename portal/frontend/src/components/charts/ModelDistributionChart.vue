<template>
  <div class="card min-w-0 p-3 sm:p-4">
    <div
      class="flex"
      :class="isMobileH5
        ? ['flex-col items-stretch gap-3', contentVisible ? 'mb-4' : '']
        : 'mb-4 items-center justify-between gap-3'"
    >
      <button
        v-if="isMobileH5"
        type="button"
      ...
      >
        ...
        <Icon
          name="chevronDown"
          size="sm"
          class="ml-2 shrink-0 text-gray-400 transition-transform"
          :class="contentVisible ? 'rotate-180' : ''"
        />
      </button>
      <h3 v-else class="text-sm font-semibold text-gray-900 dark:text-white">
        {{ !enableRankingView || activeView === 'model_distribution'
          ? t('admin.dashboard.modelDistribution')
          : t('admin.dashboard.spendingRankingTitle') }}
      </h3>
      <div v-show="contentVisible" class="flex min-w-0 flex-wrap items-center gap-2 sm:justify-end">
        <div
        ...
        <div v-if="enableRankingView" class="grid w-full grid-cols-2 rounded-lg bg-gray-100 p-1 sm:inline-flex sm:w-auto dark:bg-dark-800">
          <button
            type="button"
            class="rounded-md px-2.5 py-1 text-xs font-medium transition-colors"
            :class="
    ...
    <div
      ...
      class="flex min-w-0"
      :class="isMobileH5 ? 'flex-col items-stretch gap-4' : 'items-center gap-6'"
    >
      <div class="shrink-0" :class="isMobileH5 ? 'mx-auto h-44 w-44' : 'h-48 w-48'">
        <Doughnut :data="chartData" :options="doughnutOptions" />
      </div>
      <div class="max-h-48 flex-1" :class="isMobileH5 ? 'min-w-0 overflow-auto' : 'overflow-y-auto'">
        <table class="w-full text-xs">
          <thead>
            <tr class="text-gray-500 dark:text-gray-400">
              ...
              <th v-if="!isMobileH5" class="pb-2 text-right">{{ t('admin.dashboard.actual') }}</th>
              <th v-if="showAccountCost && !isMobileH5" class="pb-2 text-right">{{ t('admin.dashboard.accountCost') }}</th>
              <th v-if="!isMobileH5" class="pb-2 text-right">{{ t('admin.dashboard.standard') }}</th>
            </tr>
          </thead>
          <tbody>
            <template v-for="model in displayModelStats" :key="model.model">
              ...
              >
                ...
                <td v-if="!isMobileH5" class="py-1.5 text-right text-green-600 dark:text-green-400">
                  ¥{{ formatCost(customerAmountCny(model.actual_cost)) }}
                </td>
                <td v-if="showAccountCost && !isMobileH5" class="py-1.5 text-right text-orange-500 dark:text-orange-400">
                  ¥{{ formatCost(upstreamCostCny(model.account_cost)) }}
                </td>
                <td v-if="!isMobileH5" class="py-1.5 text-right text-gray-400 dark:text-gray-500">
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-1.5 text-right text-green-600 dark:text-green-400">
                ¥{{ formatCost(customerAmountCny(item.actual_cost)) }}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      callbacks: {
        label: (context: any) => {
          ...
          const formattedValue = props.metric === 'actual_cost'
            ? `¥${formatCost(customerAmountCny(value))}`
            : formatTokens(value)
    ...
    tooltip: {
      callbacks: {
        label: (context: any) => {
          ...
          const total = context.dataset.data.reduce((a: number, b: number) => a + b, 0)
          const percentage = total > 0 ? ((value / total) * 100).toFixed(1) : '0.0'
          return `${context.label}: ¥${formatCost(customerAmountCny(value))} (${percentage}%)`
        }
      }
    }
  }

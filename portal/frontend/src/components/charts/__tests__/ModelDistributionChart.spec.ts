...
describe('ModelDistributionChart', () => {
  ...
  it('keeps the original complete desktop chart layout outside H5 mode', () => {
    const wrapper = mount(ModelDistributionChart, {
      props: { modelStats },
      global: { stubs: { LoadingSpinner: true } },
    })

    expect(wrapper.find('.card').classes()).toContain('min-w-0')
    const chartContent = wrapper.findAll('.flex.min-w-0').find((element) => element.classes().includes('gap-6'))
    expect(chartContent?.classes()).toContain('items-center')

...
import { normalizeNewUsername, validateNewUsername } from '@/utils/usernamePolicy'
...
const { loading, submit } = useForm({
  form,
  submitFn: async (data) => {
    const { balance: rawBalance, email: rawEmail, username: rawUsername, role, ...rest } = data
    const balance = String(rawBalance).trim()
    const email = rawEmail.trim()
    const username = normalizeNewUsername(rawUsername)
    if (!validateNewUsername(username)) {
      throw new Error(t('auth.usernameInvalid'))
    }
    const payload: typeof rest & { email: string; username: string; role?: 'user' | 'admin'; balance?: number } = { ...rest, email, username }
  ...
  successMsg: props.successMessage === null ? undefined : (props.successMessage || t('admin.users.userCreated'))
})

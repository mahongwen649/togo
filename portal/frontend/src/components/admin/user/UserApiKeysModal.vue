    ...
    <div v-if="user" class="space-y-4">
      <div class="flex items-center gap-3 rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        <div class="flex h-10 w-10 items-center justify-center rounded-full bg-primary-100 dark:bg-primary-900/30">
          <span class="text-lg font-medium text-primary-700 dark:text-primary-300">{{ getUserAvatarInitial(user) }}</span>
        </div>
  ...
  <Teleport to="body">
    ...
    >
      <div class="max-h-64 overflow-y-auto p-1.5">
        ...
        <!-- Group options -->
        <button
          v-for="group in publicStandardGroups"
          :key="group.id"
...
const publicStandardGroups = computed(() =>
  allGroups.value.filter(group =>
    group.status === 'active' &&
    !group.is_exclusive &&
    (group.subscription_type ?? 'standard') === 'standard'
  )
)
...
const load = async () => {
  ...
  try {
    const res = await usersAPI.getUserApiKeys(props.user.id)
...
const changeGroup = async (key: ApiKey, newGroupId: number | null) => {
  ...
  try {
    const result = await apiKeysAPI.updateApiKeyGroup(key.id, newGroupId)
    // Update local data

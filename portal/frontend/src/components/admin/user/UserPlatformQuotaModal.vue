...
async function load() {
  ...
  loading.value = true
  try {
    const data = await usersAPI.getPlatformQuotas(props.user.id)
...
async function onSave() {
  ...
  try {
    ...
    await usersAPI.updatePlatformQuotas(props.user.id, payload)
...
async function onReset(platform: PlatformQuotaPlatform, quotaWindow: PlatformQuotaWindow) {
  ...
  resetting[key] = true
  try {
    const data = await usersAPI.resetPlatformQuotaWindow(props.user.id, platform, quotaWindow)
    quotas.value = normalize(data.platform_quotas || [])

...
const handleUpdateUser = async () => {
  ...
  try {
    ...
    await stepUp.run(() => usersAPI.update(userId, data))
    if (Object.keys(form.customAttributes).length > 0) await userAttributesAPI.updateUserAttributeValues(userId, form.customAttributes)

    ...
    <div v-if="user" class="space-y-6">
      ...
      <div class="flex items-center gap-4 rounded-2xl bg-gradient-to-r from-primary-50 to-primary-100 p-5 dark:from-primary-900/30 dark:to-primary-800/20">
        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-white shadow-sm dark:bg-dark-700">
          <span class="text-2xl font-semibold text-primary-600 dark:text-primary-400">{{ getUserAvatarInitial(user) }}</span>
        </div>
        <div class="flex-1">
...
const load = async () => {
  loading.value = true
  try {
    const [availableGroups, currentUser] = await Promise.all([
      groupsAPI.getAll(),
      usersAPI.getById(props.user!.id),
    ])
    // 只显示标准类型且活跃的分组
    groups.value = availableGroups.filter((g) => g.subscription_type === 'standard' && g.status === 'active')

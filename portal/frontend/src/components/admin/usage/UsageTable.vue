<template>
  <div :class="flat ? '' : 'card overflow-hidden'">
    ...
    <div :class="isMobileH5 ? 'overflow-visible' : 'overflow-auto'">
      <DataTable
        :columns="columns"
        :data="data"
        :loading="loading"
              ...
              <div v-if="row.cache_read_tokens > 0 || row.cache_creation_tokens > 0" class="flex items-center gap-2">
                <div v-if="row.cache_read_tokens > 0" class="inline-flex items-center gap-1">
                  ...
                  <span class="font-medium text-emerald-600 dark:text-emerald-400">
                    {{ t('admin.usage.cacheHitRate') }} {{ formatCacheHitRate(row) }}
                  </span>
                </div>
        ...
        <template #cell-cost="{ row }">
          <div class="text-sm">
            <div class="flex items-center gap-1.5">
              <span class="font-medium text-green-600 dark:text-green-400">¥{{ customerAmountCny(row.actual_cost).toFixed(6) }}</span>
              <span
            ...
            <div v-if="showAccountBilling && row.account_rate_multiplier != null" class="mt-0.5 text-[11px] text-orange-500 dark:text-orange-400">
              A ¥{{ upstreamCostCny(accountBilled(row)).toFixed(6) }}
            </div>
          </div>
        </template>
        ...
        <div class="space-y-1.5">
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-semibold text-blue-400">
              {{ formatMultiplier(tooltipData?.rate_multiplier ?? 1) }} × 1/7 =
              {{ formatMultiplier(customerEffectiveMultiplier(tooltipData?.rate_multiplier ?? 1)) }}x
            </span>
          </div>
          <div class="flex items-center justify-between gap-6">
          ...
          <div class="flex items-center justify-between gap-6">
            ...
            <span class="font-semibold text-green-400">¥{{ customerAmountCny(tooltipData?.actual_cost).toFixed(6) }}</span>
          </div>
...
import { customerAmountCny, customerEffectiveMultiplier, upstreamCostCny } from '@/utils/portalCurrency'
...
const cacheHitRate = (row: Pick<AdminUsageLog, 'input_tokens' | 'cache_read_tokens'>): number | null => {
  const cacheReadTokens = row.cache_read_tokens || 0
  const uncachedInputTokens = row.input_tokens || 0
  const totalInputTokens = cacheReadTokens + uncachedInputTokens
  if (cacheReadTokens <= 0 || totalInputTokens <= 0) return null
  return cacheReadTokens / totalInputTokens
}

const formatCacheHitRate = (row: Pick<AdminUsageLog, 'input_tokens' | 'cache_read_tokens'>): string => {

...
describe('admin UsageTable tooltip', () => {
  ...
  it('shows cache hit rate beside cache read tokens', () => {
    ...
    expect(wrapper.text()).toContain('76.2K')
    expect(wrapper.text()).toContain('Cache hit rate 96.9%')
  })
  ...
  it('shows service tier and billing breakdown in cost tooltip', async () => {
    ...
    expect(text).toContain('0.18 × 1/7 = 0.0257x')
    ...
    expect(text).toContain('¥0.650181')
  })
...
describe('admin UsageTable deleted-user badge', () => {
  it('renders deleted badge for a soft-deleted user row', () => {
    const row = {
      ...
      model: 'claude-3',
      user_id: 2,
      user: { id: 2, username: 'deleted-user', email: 'd@test.com', deleted_at: '2026-05-28T00:00:00Z' },
      actual_cost: 0,
      total_cost: 0,
      input_cost: 0,
      output_cost: 0,
  ...
  it('does NOT render deleted badge for an active user row', () => {
    const row = {
      ...
      model: 'claude-3',
      user_id: 3,
      user: { id: 3, username: 'active-user', email: 'active@test.com', deleted_at: null },
      actual_cost: 0,
      total_cost: 0,
      input_cost: 0,
      output_cost: 0,

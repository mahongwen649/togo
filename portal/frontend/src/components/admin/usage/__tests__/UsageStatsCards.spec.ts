...
describe('UsageStatsCards', () => {
  it('shows cache token breakdown values', () => {
    ...
    expect(text).toContain('Cache hit rate: 18.0%')
    expect(text).toContain('¥0.0010')
    expect(text).toContain('¥0.0070')
    expect(text).toContain('$0.0010')
  })

  it('updates the cache hit rate when filtered statistics change', async () => {
    const wrapper = mount(UsageStatsCards, {
      props: { stats },
    ...
    expect(wrapper.text()).toContain('Cache hit rate: 75.0%')
  })
})

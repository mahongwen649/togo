<template>
  <div class="grid grid-cols-2 lg:grid-cols-4" :class="isMobileH5 ? 'gap-2' : 'gap-4'">
    <div class="card flex min-w-0 items-center" :class="isMobileH5 ? 'gap-2 p-2' : 'gap-3 p-4'">
      <div class="shrink-0 rounded-lg bg-blue-100 dark:bg-blue-900/30 text-blue-600" :class="isMobileH5 ? 'p-1.5' : 'p-2'">
        <Icon name="document" size="md" />
      </div>
      <div class="min-w-0">
        <p class="text-xs font-medium text-gray-500">{{ t('usage.totalRequests') }}</p>
        <p class="truncate font-bold" :class="isMobileH5 ? 'text-lg' : 'text-xl'">{{ stats?.total_requests?.toLocaleString() || '0' }}</p>
        <p class="text-xs text-gray-400">{{ t('usage.inSelectedRange') }}</p>
      </div>
    </div>
    <div class="card flex min-w-0 items-center" :class="isMobileH5 ? 'gap-2 p-2' : 'gap-3 p-4'">
      <div class="shrink-0 rounded-lg bg-amber-100 dark:bg-amber-900/30 text-amber-600" :class="isMobileH5 ? 'p-1.5' : 'p-2'"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m21 7.5-9-5.25L3 7.5m18 0-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9" /></svg></div>
      <div class="min-w-0">
        <p class="text-xs font-medium text-gray-500">{{ t('usage.totalTokens') }}</p>
        <p class="truncate font-bold" :class="isMobileH5 ? 'text-lg' : 'text-xl'">{{ formatTokens(stats?.total_tokens || 0) }}</p>
    ...
    <div class="card flex min-w-0 items-center" :class="isMobileH5 ? 'gap-2 p-2' : 'gap-3 p-4'">
      <div class="shrink-0 rounded-lg bg-green-100 dark:bg-green-900/30 text-green-600" :class="isMobileH5 ? 'p-1.5' : 'p-2'">
        <Icon name="dollar" size="md" />
      </div>
      <div class="min-w-0 flex-1">
        <p class="text-xs font-medium text-gray-500">{{ t('usage.totalCost') }}</p>
        <p class="truncate font-bold text-green-600" :class="isMobileH5 ? 'text-lg' : 'text-xl'">
          ¥{{ customerAmountCny(stats?.total_actual_cost).toFixed(4) }}
        </p>
        <p class="text-xs text-gray-400">
          <template v-if="showAccountCost && totalAccountCost != null">
            <span class="text-orange-500">{{ t('usage.accountCost') }} ¥{{ upstreamCostCny(totalAccountCost).toFixed(4) }}</span>
            <span> · </span>
          </template>
          <span>
    ...
    <div class="card flex min-w-0 items-center" :class="isMobileH5 ? 'gap-2 p-2' : 'gap-3 p-4'">
      <div class="shrink-0 rounded-lg bg-purple-100 dark:bg-purple-900/30 text-purple-600" :class="isMobileH5 ? 'p-1.5' : 'p-2'">
        <Icon name="clock" size="md" />
      </div>
      <div class="min-w-0"><p class="text-xs font-medium text-gray-500">{{ t('usage.avgDuration') }}</p><p class="truncate font-bold" :class="isMobileH5 ? 'text-lg' : 'text-xl'">{{ formatDuration(stats?.average_duration_ms || 0) }}</p></div>
    </div>
  </div>
</template>

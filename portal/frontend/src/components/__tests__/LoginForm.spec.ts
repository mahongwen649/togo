...
vi.mock('@/api/admin/scope', () => ({
  getManagementScope: vi.fn().mockResolvedValue({
    is_admin: false,
  ...
  setup() {
    ...
    const handleLogin = async () => {
      if (!formData.email || !formData.password) {
        errorMessage.value = '请输入邮箱或用户名和密码'
        return
      }
...
describe('LoginForm 核心逻辑', () => {
  ...
  it('允许使用用户名提交登录', async () => {
    mockLogin.mockResolvedValue({
      access_token: 'token',
      token_type: 'Bearer',
      user: { id: 1, username: 'alice', email: 'alice@example.com', role: 'user', balance: 0, concurrency: 5, status: 'active', allowed_groups: null, created_at: '', updated_at: '' },
    })

    const wrapper = mount(LoginFormTestComponent)

    await wrapper.find('#email').setValue('alice')
    await wrapper.find('#password').setValue('password123')
    await wrapper.find('form').trigger('submit')
    await flushPromises()
  ...
  it('空表单提交显示验证错误', async () => {
    ...
    expect(wrapper.find('.error').text()).toBe('请输入邮箱或用户名和密码')

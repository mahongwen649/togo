<template>
  ...
  >
        ...
        <tr>
          ...
          <th
            ...
            :class="[
              ...
              { 'mobile-actions-collapsed': shouldCollapseMobileActions && column.key === 'actions' },
              column.class
            ]"
      ...
      <tbody class="table-body divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
        ...
        <template v-else>
          ...
          >
            ...
            >
              <button
                v-if="shouldCollapseMobileActions && column.key === 'actions'"
                type="button"
                class="mobile-action-trigger"
                :class="{ 'is-active': activeMobileActionKey === resolveRowKey(item.row, item.index) }"
                :aria-label="t('common.more')"
                :aria-expanded="activeMobileActionKey === resolveRowKey(item.row, item.index)"
                data-test="mobile-action-trigger"
                @click.stop="toggleMobileActionMenu(item.row, item.index, $event)"
              >
    ...
    <Teleport to="body">
      <div
        v-if="shouldCollapseMobileActions && activeMobileActionRow && mobileActionMenuStyle"
        ref="mobileActionMenuRef"
        class="mobile-action-menu-panel"
        :style="mobileActionMenuStyle"
        role="menu"
        data-test="mobile-action-menu"
        @click.stop="closeMobileActionMenu"
      >
        <div class="mobile-action-menu-content">
          <slot
            name="cell-actions"
            :row="activeMobileActionRow"
            :value="activeMobileActionRow.actions"
            :expanded="true"
          />
        </div>
      </div>
...
onUnmounted(() => {
  ...
  document.removeEventListener('click', closeMobileActionMenu)
  window.removeEventListener('resize', closeMobileActionMenu)
  window.removeEventListener('scroll', closeMobileActionMenu, true)
})

onMounted(() => {
  document.addEventListener('click', closeMobileActionMenu)
  window.addEventListener('resize', closeMobileActionMenu)
  window.addEventListener('scroll', closeMobileActionMenu, true)
})
...
interface Props {
  ...
  /** Collapse a long actions column into a single menu trigger on H5. */
...
const activeMobileActionKey = ref<string | number | null>(null)
const activeMobileActionRow = ref<any | null>(null)
const mobileActionMenuRef = ref<HTMLElement | null>(null)
const mobileActionMenuStyle = ref<Record<string, string> | null>(null)
const mobileActionAnchorRect = ref<DOMRect | null>(null)

const shouldCollapseMobileActions = computed(
  () => isMobileH5.value && props.collapseMobileActions && (props.actionsCount ?? 0) > 2
)

const closeMobileActionMenu = () => {
  activeMobileActionKey.value = null
  activeMobileActionRow.value = null
  mobileActionMenuStyle.value = null
  mobileActionAnchorRect.value = null
}

const positionMobileActionMenu = async () => {
  await nextTick()
  const anchor = mobileActionAnchorRect.value
  const menu = mobileActionMenuRef.value
...
const toggleMobileActionMenu = (row: any, index: number, event: MouseEvent) => {
  const key = resolveRowKey(row, index)
  if (activeMobileActionKey.value === key) {
  ...
  if (!trigger) return
  activeMobileActionKey.value = key
  activeMobileActionRow.value = row
  mobileActionAnchorRect.value = trigger.getBoundingClientRect()
  mobileActionMenuStyle.value = { left: '0px', top: '0px', visibility: 'hidden' }
  void positionMobileActionMenu().then(() => {
    if (mobileActionMenuStyle.value) {
      mobileActionMenuStyle.value = { ...mobileActionMenuStyle.value, visibility: 'visible' }
    }
  })
}
...
const shouldVirtualize = computed(() =>
  !isMobileH5.value && (sortedData.value?.length ?? 0) > (props.virtualizeThreshold ?? 100)
)
...
.table-wrapper.mobile-table :deep(th),
.table-wrapper.mobile-table :deep(td) {
...
.table-wrapper.mobile-table .mobile-actions-collapsed {
  width: 2.75rem;
  min-width: 2.75rem;
...
.table-wrapper.mobile-table th.mobile-actions-collapsed > div {
...
.mobile-action-trigger.is-active {
  background: rgb(243 244 246);
  color: rgb(17 24 39);
}

.dark .mobile-action-trigger:hover,
.dark .mobile-action-trigger.is-active {
...
.dark .mobile-action-menu-panel {
  border-color: rgb(55 65 81);
  background: rgb(31 41 55);
}

.mobile-action-menu-content :deep(> div) {
  display: flex;
...
.mobile-action-menu-content :deep(> div > button),
.mobile-action-menu-content :deep(> div > a),
.mobile-action-menu-content :deep(> div > [role='button']) {
  display: flex;
  width: 100%;
  min-height: 2.25rem;
  flex-direction: row;

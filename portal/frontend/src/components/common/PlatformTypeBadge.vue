...
type PlatformBadgePlatform = 'anthropic' | 'openai' | 'gemini' | 'antigravity' | 'grok'
type PlatformBadgeAccountType = 'oauth' | 'setup-token' | 'apikey' | 'upstream' | 'bedrock' | 'service_account'

interface Props {
  platform: PlatformBadgePlatform
  type: PlatformBadgeAccountType
  authMode?: string
  planType?: string
  privacyMode?: string
...
const planLabel = computed(() => {
  ...
  switch (normalizedPlanType.value) {
    ...
    case 'abnormal':
      return t('platformTypeBadge.subscriptionAbnormal')
    default:
...
const expiresLabel = computed(() => {
  ...
  try {
    ...
    return `${t('platformTypeBadge.subscriptionExpires')} ${yyyy}-${mm}-${dd}`
  } catch {
    return ''
  }
})
...
const privacyBadge = computed(() => {
  ...
  switch (props.privacyMode) {
    // OpenAI states
    case 'training_off':
      return { label: 'Private', icon: shieldCheck, title: t('platformTypeBadge.privacyTrainingOff'), class: 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' }
    case 'training_set_cf_blocked':
      return { label: 'CF', icon: shieldX, title: t('platformTypeBadge.privacyCfBlocked'), class: 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400' }
    case 'training_set_failed':
      return { label: 'Fail', icon: shieldX, title: t('platformTypeBadge.privacyFailed'), class: 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' }
    // Antigravity states
    case 'privacy_set':
      return { label: 'Private', icon: shieldCheck, title: t('platformTypeBadge.privacyAntigravitySet'), class: 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' }
    case 'privacy_set_failed':
      return { label: 'Fail', icon: shieldX, title: t('platformTypeBadge.privacyAntigravityFailed'), class: 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' }
    default:
      return null
  }
})
